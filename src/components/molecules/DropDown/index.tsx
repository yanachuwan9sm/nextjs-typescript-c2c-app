...
// ドロップダウンメニューの選択肢における型定義
export interface DropdownItem {
  value: string | number | null;
  label?: string;
}

// DropdownItemコンポーネントがとりうるプロパティの型定義
interface DropdownItemProps {
  item: DropdownItem;
}

// DropDownコンポーネントがとりうるプロパティの型
...
const Dropdown = (props: DropdownProps) => {
  ...
  // ドロップダウンメニューをクリックした際の選択肢のビューの表示/非表示を状態を管理するステート
  const [isOpen, setIsOpenValue] = useState(false);
  // 現在選択されている選択肢を管理するステート
  const [selectedItem, setSelectedItem] = useState(initialItem);

  // コンポーネントのルート要素のrefオブジェクト
